		...
		if (Validate::isLoadedObject($this->context->cart) && $this->context->cart->OrderExists() == false) {
			...
			foreach ($package_list as $id_address => $packageByAddress) {
				foreach ($packageByAddress as $id_package => $package) {
					...
					$order->total_tassa_di_consumo = $tassa_di_consumo_totale;
				...
				if (!$order_creation_failed && isset($order->id)) {
					...
					if ($id_order_state != Configuration::get('PS_OS_ERROR') && $id_order_state != Configuration::get('PS_OS_CANCELED') && $this->context->customer->id) {
						...
						$data = array(
								...
								'{total_tassa_di_consumo}' => Tools::displayPrice($order->total_tassa_di_consumo, $this->context->currency, false),
								'{total_tax_paid}' => Tools::displayPrice(($order->total_products_wt - $order->total_products) + ($order->total_shipping_tax_incl - $order->total_shipping_tax_excl) - $order->total_tassa_di_consumo, $this->context->currency, false));
