...
product_tabs['Combinations'] = new function(){
		...
		function editProductAttribute (url, parent){
			$.ajax({
				...
				success: function(data) {
					...
					var tassa_di_consumo_comb = data[0]['tassa_di_consumo_comb'];
					var ml_flacone_comb = data[0]['ml_flacone_comb'];
					var ml_nicotina_comb = data[0]['ml_nicotina_comb'];
					var codice_articolo = data[0]['codice_articolo'];
	...
	tassa_di_consumo_comb, ml_flacone_comb, ml_nicotina_comb, codice_articolo)
	{
		...
		if (tassa_di_consumo_comb == 1)
			getE('attribute_tassa_di_consumo_comb').checked = true;
		else
			getE('attribute_tassa_di_consumo_comb').checked = false;
		
		getE('attribute_ml_flacone_comb').value = parseFloat(ml_flacone_comb).toFixed(2);
		getE('attribute_ml_nicotina_comb').value = parseFloat(ml_nicotina_comb).toFixed(2);
		getE('attribute_codice_articolo').value = codice_articolo;
