<?php
  ...
  class DB
  {
    ...
    public function __construct($base, $server, $user, $pass)
    {
      ...
      $this->connection = new mysqli($server, $user, $pass, $base);
    ...
    /** Query the database.
      ...
      * @return The result of the query, to use with fetchNextObject() or fetchNextAssoc() or fetchNextArray().
      */
     ...
     * @param $query The query.
     * @param $debug If true, it output the query and the resulting table.
     * @return The result of the query as an array, to read with a foreach($result as $row)
     */
    public function queryS($query, $debug = -1)
    {
    	$this->nbQueries++;
    	$this->lastResult = $this->connection->query($query) or $this->debugAndDie($query);
    	...
    	if (method_exists($this->lastResult, 'fetch_all')) {
    ...
    {
      $this->nbQueries++;
      $this->connection->query($query) or $this->debugAndDie($query);
	...
	/** Convenient method for mysql_fetch_assoc().
      ...
      * @return An object representing a data row.
      */
    public function fetchNextAssoc($result = NULL)
    {
      if ($result == NULL)
        $result = $this->lastResult;
